...
func (ip Addr) Prefix(b int) (Prefix, error) {
	...
	case z4:
		if b > 32 {
			return Prefix{}, errors.New("prefix length " + Itoa(b) + " too large for IPv4")
		}
		effectiveBits += 96
	default:
		if b > 128 {
			return Prefix{}, errors.New("prefix length " + Itoa(b) + " too large for IPv6")
		}
	}
...
func (ip *Addr) UnmarshalBinary(b []byte) error {
	...
	case n > 16:
		var v [16]byte
		copy(v[:], b[:16])
		*ip = AddrFrom16(v).WithZone(string(b[16:]))
		return nil
	}
...
func (p AddrPort) Compare(p2 AddrPort) int {
	...
	return CompareUint16(p.Port(), p2.Port())
}
...
func (p Prefix) compare(p2 Prefix) int {
	if c := Compare(p.Addr().BitLen(), p2.Addr().BitLen()); c != 0 {
		return c
	}
	if c := Compare(p.Bits(), p2.Bits()); c != 0 {
		return c
	}
...
func (p Prefix) String() string {
	...
	return p.ip.String() + "/" + Itoa(p.Bits())
}
